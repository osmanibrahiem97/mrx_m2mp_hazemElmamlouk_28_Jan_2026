...
Widget SellerOrderViewRefundPriceDetails(SellerOrdersViewRefundMpModel model, BuildContext context,
    AppLocalizations? localizations) {
  return sellerOrderHeaderLayout(
      context,
      localizations?.translate(MpStringConstants.refundTotals) ?? "",
      Padding(
        padding: const EdgeInsets.symmetric(horizontal: AppSizes.size10),
        child: Column(
          children:
            (model.totals??[]).map((e) => SellerOrderViewRefundPriceDetailsItem(
                e.label ?? "",
                e.formattedValue??"",
                context)
...
Widget SellerOrderViewRefundPriceDetailsItem(String key, String value, BuildContext context) {
  return Padding(
