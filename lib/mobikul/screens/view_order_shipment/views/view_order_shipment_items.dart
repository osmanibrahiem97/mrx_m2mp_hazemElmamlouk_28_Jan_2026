...
Widget ViewOrderShipmentItems(
  ...
  List<Widget> getOptionWidgets(List<dynamic>? options) {
    List<Widget> widgets = [];
    if (options != null) {
      for (var option in options) {
        if (option is Map<String, dynamic>) {
          String label = option['label'] ?? '';
          String value = (option['value'] as List<dynamic>).join(", ");
          widgets.add(getListItem(label, value));
          widgets.add(const SizedBox(height: AppSizes.size8));
        }
      }
    }
    return widgets;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            getListItem(
                Utils.getStringValue(
                    context, AppStringConstant.productName),
                item?.name.toString()),
            const SizedBox(
              height: AppSizes.size8,
            ),
