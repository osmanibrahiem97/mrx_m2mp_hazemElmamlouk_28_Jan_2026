...
class _profileState extends State<ProfileScreen> {
  ...
  void initState() {
    ...
    if(AppConstant.isMarketplace == true){
      profileScreenBloc?.add(SellerMenuEvent(false));
    }
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<ProfileScreenBloc, ProfileScreenState>(
        ...
        } else if(currentState is SellerMenusState){
          isLoading = false;
          sellerMenuData = currentState.model;
          // WidgetsBinding.instance?.addPostFrameCallback((_) {
    ...
    return RefreshIndicator(
      ...
      onRefresh: () {
        ...
        return Future.delayed(Duration.zero).then((value) {
          HiveStore().reset();
          profileScreenBloc?.add(const SellerMenuEvent(true));
        });
      },
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            Stack(
              children: [
                // RefreshIndicator now wraps the scrollable widget properly
                Column(
                  children: [
                    profileMenu(
                          () {
