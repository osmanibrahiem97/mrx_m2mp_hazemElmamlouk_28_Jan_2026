...
class _HomeScreenState extends State<HomeScreen> with AutomaticKeepAliveClientMixin<HomeScreen> {
              ...
              case "image":
                {
                  homePageWidgets?.add(Column(
                    children: [
                      space(),
                      HomeBanners((item?.banners ?? []), false,item?.color??'',item?.image??''),
                    ],
                  ));
                  break;
                }
              ...
              case "banner":
                {
                  homePageWidgets?.add(Column(
                    children: [
                      widgetSpace(0,AppSizes.paddingGeneric),
                      HomeBanners((item?.banners ?? []), true,item?.color??'',item?.image??''),
                    ],
                  ));
                  break;
                }
  ...
  List<SortOrder>? getSortedCarouselsData(List<SortOrder>? sortedOrder) {
    ...
    sortOrder.sort((a, b) => int.parse(a.position!.replaceAll(",", "")??"").compareTo(int.parse(b.position!.replaceAll(",", "")??"")));
    return sortOrder;
  }

  void addProductCarousel(Carousel carousel, List<dynamic>? customerWishlist) async {
  ...
  void setOnBoardingVersion() {
    if (homePageData?.walkthroughVersion?.isNotEmpty ?? false) {
      if (double.parse(appStoragePref.getWalkThroughVersion().toString()??"") < double.parse(homePageData?.walkthroughVersion.toString()??"0.0")) {
        appStoragePref.setShowWalkThrough(true);
        appStoragePref.setWalkThroughVersion(homePageData?.walkthroughVersion.toString()?? "");
      }
    }
  }
  ...
  void setMarketPlaceData() {
    appStoragePref.setIsAdmin(homePageData?.isAdmin ?? false);
    appStoragePref.setIsSeller(homePageData?.isSeller ?? false);
    appStoragePref.setIsDisapproved(homePageData?.isDisapproved ?? false);
    appStoragePref.setIsPendingSeller(homePageData?.isPending ?? false);
    appStoragePref.setShowSellerProfile(homePageData?.isSellerProfileVisible ?? false);
  }
}
