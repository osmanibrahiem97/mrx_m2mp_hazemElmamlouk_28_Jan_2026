...
import '../models/invoice_view/download_invoice_model.dart';
...
/// Log response
  ...
  Future<SignUpScreenModel?> createAccountFormData() async {
    var response = await (client.clientToQuery()).query(QueryOptions(
      ...
      fetchPolicy: FetchPolicy.noCache,
      document: gql(
        (AppConstant.isMarketplace) ? mutation.createAccountFormDataMp(appStoragePref.getStoreId()) : mutation.createAccountFormData(appStoragePref.getStoreId()),
      ),
    ));
    return handleResponse(
      response,
 ...
 ///  Wishlist Sharing
  ...
  Future<DownloadInvoiceModel?> getDownloadUrl(
    ...
    return handleResponse(
      response,
      'downloadallinvoice',
          (json) => DownloadInvoiceModel.fromJson(json),
    );
  }
