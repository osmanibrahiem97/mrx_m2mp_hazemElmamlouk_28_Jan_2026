...
package com.webkul.magento.mlkit.activities
...
import com.example.test_new.databinding.ActivityCameraSearchBinding
import com.google.android.gms.common.annotation.KeepName
import com.google.mlkit.vision.label.ImageLabel
import com.google.mlkit.vision.label.defaults.ImageLabelerOptions
import com.google.mlkit.vision.text.Text
import com.webkul.magento.mlkit.adapters.CameraSearchResultAdapter
import com.webkul.magento.mlkit.customviews.CameraSource
import com.webkul.magento.mlkit.labeldetector.LabelDetectorProcessor
import com.webkul.magento.mlkit.textdetector.TextRecognitionProcessor
...
class CameraSearchActivity : AppCompatActivity(), ActivityCompat.OnRequestPermissionsResultCallback,
        ...
        for (eachBlock in blocks) {
            for (eachLine in eachBlock.lines) {
                for (eachElement in eachLine.elements) {
                    if (!displayList.contains(eachElement.text) && displayList.size <= 20) {
    ...
    fun updateSpinnerFromResults(labelList: List<ImageLabel>) {
        for (visionLabel in labelList) {
            if (!displayList.contains(visionLabel.text) && visionLabel.confidence > 0.5f && displayList.size <= 20) {
